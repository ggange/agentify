...
export function renderAnnotationGuide(files: FileAnnotations[]): string {
  const lines: string[] = [
    ...
    '',
    'Generated by [agentify](https://github.com/ggange/agentify). This file identifies',
    'content in your codebase that would benefit from explicit `<AIContent>` annotation,',
    'so agentify can extract it into AI-readable markdown.',
    '',
    '## How it works',
    '',
    '`<AIContent>` (from `@pkg/react`) is a React Fragment wrapper — **zero DOM elements,',
    'zero layout impact.** It marks content for agentify to include in per-route `.md` files.',
    'Once you annotate, re-run `npx agentify` and your previously-empty pages will have content.',
    '',
    '## Status: `@pkg/react` is not yet published',
    '',
    '**Do not run the prompt below yet.** `@pkg/react` is in progress and will be published soon.',
    'Watch [github.com/ggange/agentify](https://github.com/ggange/agentify) for the release.',
